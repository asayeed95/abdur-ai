...
tldr: "The handoff doc said the site was 85% done — finish wiring and deploy. The last 15% took five fixes, and every one had been sitting in the 'done' pile: the gate that would catch it hadn't run yet. npm install failed on a next-15 / React-19 peer conflict; a committed .npmrc fixed it. The dev server rendered five of seven routes, but the homepage and /llms.txt 500'd — YAML had parsed an unquoted frontmatter date into a Date object the string type never saw; one ISO-coercion helper in the loader closed both. `next build` then surfaced two latent type errors dev never checked: a missing `id` prop on the Reveal wrapper and React 19's removal of the global JSX namespace. The build passed and Vercel still refused to deploy: Next.js 15.0.3 (CVE-2025-29927) and next-mdx-remote 5.0.0 are blocked at the platform gate; upgrades to 15.5.19 and 6.0.0 cleared it. The handoff doc even advised downgrading next-mdx-remote to 4.4.1 — toward the vulnerability. Local-green is not deploy-green; 'done' only counts at the strictest gate."
