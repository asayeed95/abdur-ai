...
Here's the warm path Mnemix is designed for sub-300ms voice recall on:
...
That is the design target, end to end, on the warm path — and it fits inside the 200–500ms of VAD silence the agent was going to wait through regardless. The caller never perceives a pause, because there is no added pause — the lookup happens during silence that already existed.
