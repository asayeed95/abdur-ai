# Archived from /Users/mental/Projects/abdur-ai/CLAUDE.md on 2026-08-19 — read on demand only

The lean `CLAUDE.md` keeps: repo identity/purpose, the 8 numbered build-discipline rules (read PROCESS.md + build-plan.md first, Phase-tag responses, check-phase.sh, the NEVER-SKIP design/content lock gate, verify-before-report, branch/staging discipline, PR checklist, AGENTS.md/PLAYBOOK.md pointers), core commands, pointers to standing law (Linear-first, Autonomy Doctrine, public-claims law, secrets rule) plus a PROCESS.md-supersedes-CLAUDE.md precedence note, and the locked-files list (now including the `<Reveal>`-only and MDX-only-posts items). Everything below — the original scaffolding narrative, the numbered "What you need to finish" task list, the "Architecture decisions" prose, the distribution kit, and the closing pitch — is history/detail. **Cross-checked against the live repo on 2026-08-19: tasks 2, 4, 5, and 6 below are DONE** (see `docs/superpowers/specs/build-plan.md` D-1 through D-4) even though this archived prose still frames them as open — `build-plan.md` is the authoritative live task list, not this file. Task 3 (Supabase persistence, `build-plan.md` W-1) and task 8 (Stripe, W-3) are still genuinely open. Task 7 (deploy) is mostly done — production is live per `build-plan.md` W-2, with only the `www` CNAME check and social-card validators still outstanding.

---

## Build-discipline preamble (verbatim, original opening paragraph before the 8 numbered rules)

This repo runs a **light** build-discipline gate — right-sized for a near-done public content site (no auth, no live payments). Full rules: **`PROCESS.md`**; `PROCESS.md`'s own detail supersedes anything in `CLAUDE.md` if they ever conflict. (Lean `CLAUDE.md` now states the precedence half of this directly; this paragraph is the original verbatim source.)
...
## What's done (as of the original 2026-07-16 writeup)
...
## What you need to finish (original numbered list)
...
### 2. Wire the subscribe form to a real provider (30 min) — DONE, see build-plan.md D-1
...
### 3. Persist the agent webhooks (Supabase) (1-2 hours) — STILL OPEN, build-plan.md W-1
...
### 4. Generate the OG cover image (15 min) — DONE, see build-plan.md D-2
...
### 5. Replace placeholders (5 min) — DONE, see build-plan.md D-3
...
### 6. Add the other 3 written posts (30 min) — DONE, see build-plan.md D-4
...
### 7. Deploy to Vercel (15 min) — MOSTLY DONE, see build-plan.md W-2
...
### 8. (Optional, Phase 2) Stripe + the Library — STILL OPEN/DEFERRED, build-plan.md W-3
...
## Path note (machine-dependent, verified 2026-08-19)

The original file's shell commands and content-vault paths (`/Users/agencyflow/Projects/abdur-ai`, `/Users/agencyflow/Documents/abdur.ai/abdur-ai-content/...`) are macmini paths (user `agencyflow`). On this machine (asec-god, user `mental`), the repo checkout is `/Users/mental/Projects/abdur-ai`; the content-vault path has not been re-verified here. Do not paste `/Users/agencyflow/...` paths into commands on this machine without checking first.
